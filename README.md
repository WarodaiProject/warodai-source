...
В редких случаях переводы, имеющие общее пояснение (например, в виде стилистической пометы), могут объяединяться в группу при этом пояснении. Внутри такой группы переводы разделяются буквой со скобкой. 
...
### 3.9.6 Указание управления глагола (`(に), (を)` и т.п.)
...
### 3.9.7 Транслитерация вместо перевода
...
### 3.9.8 Случаи отсутствие перевода
...
#### 3.9.8.1 Служебный элемент, формант, гоноратив
...
#### 3.9.8.2 Связанность употребления в словосочетаниях (`<i>связ.:</i>`) и ономатопоэтизмы (`<i>ономат.:</i>`)
...
#### 3.9.8.4 Замена перевода производным (`: ～する`, `: ～の` и .т.п.)
...
#### 3.9.8.5 Пометы при опущенном перевода
...
### 3.9.9 Исключительные написания (`тк.`)
...
### 3.9.10 Дополнительные написания (`тж.`)
...
### 3.9.11 Устаревшие дополнительные написания (`уст.`)
...
### 3.9.12 Сочетание исключительных и дополнительных написаний
...
### 3.9.13 Подстановочные ссылки (`см.`)
...
#### 3.9.13.1 Оформление подстановочных ссылок
...
#### 3.9.13.2 Подстановочные ссылки на части статей
...
#### 3.9.13.3 Стилистические пометы при подстановочных ссылках (`уст. см`, `кн. см` и т.п.)
...
### 3.9.16 Отсылка к полному варианту слова (`сокр.`)
...
### 3.9.18 Стилистические пометы (`кн.`, `разг.`, `эпист.` и др.)
...
### 3.9.19 Пометы, обозначающие специальные области (*бот.*, *мед.* и т. п.)
...
### 3.9.20 Заимствованные слова (`англ.`, `нем.` и т.п.)
...
### 3.9.21 Биологическая номенклатура
...
Отдельным случаем псевдопроизводных следует считать именные прилагательные, которые помещаются в заголовке в форме основы, но переводятся в определительной форме, т.е. с такими формантами как *な*, *の*, *なる*, *たる*. Эти форманты не образуют самостоятельных слов, но образуют форму, удобную для русского перевода. Оформление таких случаев совпадает с оформлением обычных производных, стоящих на месте перевода.  
...
В редких случаях факультативная часть может быть указана при отсутствии других формантов. В таком случае это следует понимать, как указание на то, что в данной рубрике перевод, указанный при производном слове, подходит и для заглавного слова тоже.
...
Также, как и в переводах (см. раздел 3.9.2) буквой со скобкой могут разделяться переводы производного внутри группы при общем пояснении (чаще всего - помете).
...
в) представить устойчивые словосочетания и идиоматические выражения.
    ...
    1) овощи;
    2) <i>см.</i> <a href="#003-62-00">とうな</a>;
    3) <i>см.</i> <a href="#004-11-79">かぶら</a>;
...
Последними следуют идиоматические выражения, значение которых не выводится напрямую из значений входящих в них слов. Такие выражения обозначаются ромбом `◇` (U+25C7, White Diamond). При этом такое выражение помещается в рубрику на основании семантической или грамматической близости значения заглавного слова в нем к значению рубрики.  
...
Оформление написания выражения во многом аналогично оформлению модели производного. В нем могут использоваться следующие обозначения и способы компактной записи, использующиеся в моделях производных:
...
В конце рубрики, на отдельной строке могут даваться общие отсылки с пометой `ср.`. В отличие от отсылок в переводах (см. раздел 3.9.14), общие отсылки относятся ко всей рубрике целиком, включая все производные и выражения. Оформление общих отсылок аналогично описанному в разделе 3.9.14.

...
### 3.9.5 Указание управления глагола (`(に), (を)` и т.п.)
...
Перевод производного дается после модели и оформляется в соответствии с правилами оформления, описанными в разделе 3.9. Таким образом, в переводе могут использоваться: разделение переводов запятыми и точкой с запятой, пояснения курсивом, пометы, круглые и квадратные скобки, подстановочные ссылки отсылки и т.п.
...
Следует обратить внимание, что внутри групп варианты переводов могут разделяться запятой или точкой с запятой. 
...
Последними следуют идиоматические выражения, значение которых не выводится напрямую из значений входящих в них слов. При этом такое выражение помещается в рубрику на основании семантической или грамматической близости значения заглавного слова в нем к значению рубрики.  
...
а) многоточие *…* (U+2026, Horizontal Ellipsis) - для обозначения синтаксических валентностей;
    ...
    だっする【脱する】(дассуру)〔004-65-29〕
    <...>
    2) освобождать[ся] <i>от чего-л.</i>; выходить, выбираться <i>(напр. из трудного положения)</i>; избегать <i>чего-л.</i>;
    …はまだ農業国より脱しない <i>какое-л. государство</i> продолжает оставаться аграрной страной;
    <...>
...
в) круглые и квадратные скобки (`()`, `[]`) - для компактной записи вариантов выражений.
    ...
    たいか【対価】(тайка)〔005-39-18〕
    1) эквивалент; компенсация;
    対価として в компенсацию <i>за что-л.</i>; в обмен <i>на что-л.</i>;
    2) цена;
    対価の決定(維持) стабилизация цен.
...
#### 3.11.1.1 Обозначение чтений в написаниях выражений (`//`)
В косые скобки (`//`) заключается в выражениях чтение иероглифа (катаканой) в тех редких случаях, когда:

а) при наличии у иероглифа нескольких чтений, окуригана не позволяет с полной уверенностью выбрать одно из них;

    ぐんせき【軍籍】(гунсэки)〔004-80-48〕
    список личного состава армии (флота);
    軍籍に入/ハイ/る(身を置く) вступать на военную службу (в армию, во флот).

б) чтение нужно для наглядного отделения одного слова от другого, например: 人は一代イチダイ名は末代.